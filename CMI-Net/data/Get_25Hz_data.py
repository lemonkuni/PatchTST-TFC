...
def Data_Segm(df_data, single=True, tri=False):
  ...
  for s in segments:
    ...
    for j in range(0,len(data_segment),100):
      # 修改为使用加速度计数据(Ax,Ay,Az)，并每4个点取1个
      temp_sample = data_segment[['Ax','Ay','Az']].iloc[j:j+200:4,:].values
      if len(temp_sample) == 50:  # 修改长度检查为50
...
def get_data(train_subjects = [2,3,7,8], valid_subject = 11, test_subject = 14):
    # 根据操作系统选择数据路径
    path = 'E:\\program\\aaa_DL_project\\0000PatchTST-TFC\\CMI-Net\\data\\Acc_Gyr_Data.csv'
